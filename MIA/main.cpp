...
void enterConversation(bool *conversing) {
    ...
    while (true) {
	    ...
	    *conversing = false;
        ...
        AgentResponse assistantResponse = AgentManager::getInstance().generateContent(agentId, userPrompt);

        if (assistantResponse.success) {
            std::cout << "Assistant: " << assistantResponse.generatedText << std::endl;
        } else {
            std::cerr << "Assistant Error: " << assistantResponse.errorMessage << " (HTTP " << assistantResponse.httpStatusCode << ")" << std::endl;
            // Optionally, you might want to break or handle errors more gracefully here
        }
    }
